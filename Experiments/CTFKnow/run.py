...
# Locally-deployed Pen-SLM (see Experiments/server.py) exposes an OpenAI-compatible
# /v1/chat/completions endpoint. Passing one of these names via `-l` routes the
# evaluation at that local server instead of a hosted API.
LOCAL_MODEL_NAMES = {"pen-slm", "pen-slm-local"}
LOCAL_MODEL_BASE_URL = os.environ.get("PEN_SLM_SERVER_URL", "http://localhost:8083/v1")
...
class Envaluation:
    ...
    def envaluate(self):
        ...
        # NOTE: previously this assigned to a local `client` only inside the Groq
        # branch, which made the module-level `client` unreachable below (Python
        # scoping treats `client` as local to this function once it's assigned
        # anywhere in it) -- any non-Groq model would hit UnboundLocalError.
        if self.llm in ["llama3-70b-8192", "mixtral-8x7b-32768"]:
            chat_client = Groq(
                    api_key=os.environ["GROQ_API_KEY"],
                )
        elif self.llm in LOCAL_MODEL_NAMES:
            chat_client = OpenAI(api_key="not-needed", base_url=LOCAL_MODEL_BASE_URL)
        else:
            chat_client = client
...
def main():
    ...
    if args.command == 'K':
        ...
        # `-o` doubles as the question-list file: Envaluation reads it at init and
