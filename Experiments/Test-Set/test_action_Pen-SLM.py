# -*- coding: utf-8 -*-
"""
Evaluation script for the trained Action Model (Qwen3-14B + LoRA).
Runs inference on output_test_data.csv and scores each sample with:
...
Judge model: gpt-4o-mini  (set OPENAI_API_KEY in environment)

Requirements:
    pip install transformers peft accelerate bitsandbytes pandas numpy tqdm torch openai nest_asyncio
"""
...
# ─────────────────────────── Inference ─────────────────────────────────────
def build_prompt(ptt: str, strategy: str, explanation: str) -> str:
    user_text = (
        f"<PTT>{ptt}</PTT>\n"
...
def main():
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating"):
        ...
        #gen_action   = sections["think"] + sections["action"]
